...
await page.goto('http://127.0.0.1:4179/');await page.getByRole('button',{name:'Play films',exact:true}).first().click();await page.waitForFunction(()=>document.querySelector('video')?.currentTime>.1);assert.match(await page.locator('video').first().getAttribute('src'),/-mobile.mp4$/);log.push('Explicit mobile playback uses the mobile video file.');
await page.emulateMedia({reducedMotion:'reduce'});await page.setViewportSize({width:1440,height:900});await page.goto('http://127.0.0.1:4179/');assert.equal(await page.locator('video').count(),0);assert.equal(await page.locator('.hero-film').evaluate(e=>getComputedStyle(e).animationName),'none');log.push('Reduced motion disables autoplay and transitions.');
